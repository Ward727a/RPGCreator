...
static func make_static_signal(object_to_assign_signal, signal_name: String) -> Signal:
	if object_to_assign_signal.has_user_signal(signal_name): # Check if the signal already exists
		return Signal(object_to_assign_signal, signal_name) # If it exists, return the signal
	object_to_assign_signal.add_user_signal(signal_name) # If it doesn't exist, create the signal on the object
	return Signal(object_to_assign_signal, signal_name) # Return the signal
