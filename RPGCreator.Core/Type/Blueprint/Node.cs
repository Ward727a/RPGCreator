...
public abstract class Node : ISerializable, IDeserializable
{
    ...
    {
        info.TryGetValue(nameof(Id), out var id, string.Empty, "Error while deserializing Node.Id");
        Id = id;
        info.TryGetValue(nameof(OpCode), out EGraphOpCode opCode, EGraphOpCode.none, "Error while deserializing Node.OpCode");
        OpCode = opCode;
        info.TryGetValue(nameof(DisplayName), out var displayName, string.Empty, "Error while deserializing Node.DisplayName");
        DisplayName = displayName;
        info.TryGetValue(nameof(Description), out var description, string.Empty, "Error while deserializing Node.Description");
        Description = description;
        info.TryGetValue(nameof(Path), out var path, string.Empty, "Error while deserializing Node.Path");
        Path = path;
        info.TryGetValue(nameof(X), out double x, 0D, "Error while deserializing Node.X");
        X = x;
        info.TryGetValue(nameof(Y), out double y, 0D, "Error while deserializing Node.Y");
        Y = y;
        
        Inputs.Clear();
        Outputs.Clear();
        
        info.TryGetList(nameof(Inputs), out List<Port> inputs, new List<Port>(), "Error while deserializing Node.Inputs");
        Inputs.AddRange(inputs);
        
        info.TryGetList(nameof(Outputs), out List<Port> outputs, new List<Port>(), "Error while deserializing Node.Outputs");
