...
public sealed class Link : ISerializable, IDeserializable
{
    ...
    {
        info.TryGetValue(nameof(FromNodeId), out var fromNodeId, string.Empty, "Error while deserializing Link.FromNodeId");
        FromNodeId = fromNodeId;
        info.TryGetValue(nameof(ToNodeId), out var toNodeId, string.Empty, "Error while deserializing Link.ToNodeId");
        ToNodeId = toNodeId;
        info.TryGetValue(nameof(FromPortId), out var fromPortId, string.Empty, "Error while deserializing Link.FromPortId");
        FromPortId = fromPortId;
        info.TryGetValue(nameof(ToPortId), out var toPortId, string.Empty, "Error while deserializing Link.ToPortId");
        ToPortId = toPortId;
    }
}