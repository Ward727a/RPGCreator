...
public sealed class GraphDocument : ISerializable, IDeserializable
{
    ...
    public struct NodeData() : IDeserializable, ISerializable
    {
        ...
        { 
            info.TryGetValue(nameof(Fullpath), out string fullpath, string.Empty, "Error while deserializing NodeData.fullpath");
            info.TryGetValue(nameof(Id), out string id, string.Empty, "Error while deserializing NodeData.id");
            info.TryGetValue(nameof(X), out double x, 0D, "Error while deserializing NodeData.X");
            info.TryGetValue(nameof(Y), out double y, 0D, "Error while deserializing NodeData.Y");
            
            info.TryGetList(nameof(Inputs), out List<PortData> inputs, new List<PortData>(), "Error while deserializing NodeData.inputs");
            info.TryGetList(nameof(Outputs), out List<PortData> outputs, new List<PortData>(), "Error while deserializing NodeData.outputs");
    ...
    public struct PortData() : IDeserializable, ISerializable
    {
        ...
        public void SetObjectData(DeserializationInfo info)
        {
            info.TryGetValue(nameof(Id), out string id, string.Empty, "Error while deserializing PortData.id");
            info.TryGetValue(nameof(Name), out string name, string.Empty, "Error while deserializing PortData.name");
            info.TryGetValue(nameof(Value), out object? value, null, "Error while deserializing PortData.value");
    ...
    public void SetObjectData(DeserializationInfo info)
    {
        ...
        info.TryGetList(nameof(Nodes), out List<NodeData> nodes, new List<NodeData>(), "Error while deserializing GraphDocument.Nodes");
        ...
        info.TryGetList(nameof(Links), out List<Link> links, new List<Link>(), "Error while deserializing GraphDocument.Links");
        Links.Clear();
        foreach (var link in links)
        {
            AddLink(link);
