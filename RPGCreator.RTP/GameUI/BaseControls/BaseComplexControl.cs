...
/// <summary>
/// A complex control is a control composed of multiple internal components.<br/>
/// The internal components are not directly accessible from the outside, and are managed by the complex control itself.<br/>
/// If you want to create a control that can contain other controls and allow external access to them, consider using a <see cref="ContainerControl"/> instead.<br/>
/// Complex controls are useful for creating controls that have a specific structure and behavior, such as a text box with a background and a cursor, or a button with an icon and a label.
/// </summary>
public abstract class BaseComplexControl : BaseControl
{
    ...
    protected override string _Name { get; set; } = "BaseComplexControl";
    ...
    private bool _isBatchOpened = false;
    ...
    protected void BeginContainerDraw()
    {
        ...
        Rectangle newScissor = Rectangle.Intersect(LastScissor, contentArea);
    
        if (newScissor != LastScissor)
        {
            if(Parent != null)
            ...
            Renderer.SpriteBatch.Begin(
                rasterizerState: Renderer.ClippingRasterizerState, transformMatrix: Matrix.Identity
            );
    ...
    {
        if (!ClipToBounds || !_isBatchOpened) return;
        ...
        ref int verticalWheelDelta, ref int horizontalWheelDelta, ref bool isHandled)
    {
        foreach (var internalComponent in _internalComponents)
        {
            internalComponent.UpdateMouseInput(mousePosition, isLeftButtonDown, isMiddleButtonDown, isRightButtonDown, ref verticalWheelDelta, ref horizontalWheelDelta, ref isHandled);
            if (isHandled) break;
        }
