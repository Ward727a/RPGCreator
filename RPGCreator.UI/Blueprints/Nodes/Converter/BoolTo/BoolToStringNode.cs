...
public class BoolToStringNode : BaseNodeLogic
{
    public override FlowType FlowType => FlowType.Pure;
    public override string Title => "Bool to String";
    public override PipedPath Category { get; } = "Converter".ToPipedPath().Extend("Bool To");
    public override URN Urn { get; } = DefaultUrnModule.ToUrnModule("rpgc").ToUrn("bool_to_string_node");

    public override IReadOnlyList<IConnectorLogic> Inputs { get; set; } = new List<IConnectorLogic>
        { new BoolConnectorLogic("Value to convert") };
    ...
    public override void GenerateCode(CodeContext context, IndentedTextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("// BOOL TO STRING");
        if(Inputs[0].RuntimeHasParent)
        {
            writer.WriteLine($"string {varName} = {input} ? \"true\" : \"false\";");
        }
        else
        {
            ...
            {
                Logger.Error("ERROR WHILE GENERATING CODE: BOOL TO STRING NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine("// ERROR WHILE GENERATING CODE: BOOL TO STRING NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine($"string {varName} = \"false\";");
                return;
            }
            if(context.InDebugMode)
                writer.WriteLine("// Optimized by compiler: pin was not linked, using node value");
            writer.WriteLine($"string {varName} = {(rawValue.Value ? "\"true\"" : "\"false\"")};");
        }
    }
}