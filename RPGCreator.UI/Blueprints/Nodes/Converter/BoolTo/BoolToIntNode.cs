...
public class BoolToIntNode : BaseNodeLogic
{
    public override FlowType FlowType => FlowType.Pure;
    public override string Title => "Bool to Int";
    public override PipedPath Category { get; } = "Converter".ToPipedPath().Extend("Bool To");
    public override URN Urn { get; } = DefaultUrnModule.ToUrnModule("rpgc").ToUrn("bool_to_int_node");
    ...
    public override IReadOnlyList<IConnectorLogic> Outputs { get; set; } = new List<IConnectorLogic> { new IntConnectorLogic("Converted value") };
    public override void GenerateCode(CodeContext context, IndentedTextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("// BOOL TO INT");
        if(Inputs[0].RuntimeHasParent)
        {
            writer.WriteLine($"int {varName} = {input} ? 1 : 0;");
        }
        else
        {
            ...
            {
                Logger.Error("ERROR WHILE GENERATING CODE: BOOL TO INT NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine("// ERROR WHILE GENERATING CODE: BOOL TO INT NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine($"int {varName} = 0;");
                return;
            }
            if(context.InDebugMode)
                writer.WriteLine("// Optimized by compiler: pin was not linked, using node value (true: 1, false: 0)");
            writer.WriteLine($"int {varName} = {(rawValue.Value ? "1" : "0")};");
        }
    }
}