...
public class IntToFloatNode : BaseNodeLogic
{
    public override FlowType FlowType => FlowType.Pure;
    public override string Title => "Int to Float";
    public override PipedPath Category { get; } = "Converter".ToPipedPath().Extend("Int To");
    public override URN Urn { get; } = DefaultUrnModule.ToUrnModule("rpgc").ToUrn("int_to_float_node");
    
    public override IReadOnlyList<IConnectorLogic> Inputs { get; set; } = new List<IConnectorLogic> { new IntConnectorLogic("Value to convert") };
    ...
    public override void GenerateCode(CodeContext context, IndentedTextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("// INT TO FLOAT");
        if (Inputs[0].RuntimeHasParent)
        {
            writer.WriteLine($"float {varName} = (float){input};");
        }
        else
        {
            ...
            {
                Logger.Error("ERROR WHILE GENERATING CODE: INT TO FLOAT NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine("// ERROR WHILE GENERATING CODE: INT TO FLOAT NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine($"float {varName} = -1f;");
                return;
            };
            
            if(context.InDebugMode)
                writer.WriteLine("// Optimized by compiler: pin was not linked, using node value");
            writer.WriteLine($"float {varName} = {rawValue.Value};");
        }
    }
}