...
public class IntToBoolNode : BaseNodeLogic
{
    public override FlowType FlowType => FlowType.Pure;
    public override string Title => "Int to Bool";
    public override PipedPath Category { get; } = "Converter".ToPipedPath().Extend("Int To");
    public override URN Urn { get; } = DefaultUrnModule.ToUrnModule("rpgc").ToUrn("int_to_bool_node");
    ...
    public override void GenerateCode(CodeContext context, IndentedTextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("// INT TO BOOL");
        if (Inputs[0].RuntimeHasParent)
        {
            writer.WriteLine($"bool {varName} = {input} != 0;");
        }
        else
        {
            ...
            {
                Logger.Error("ERROR WHILE GENERATING CODE: INT TO BOOL NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine("// ERROR WHILE GENERATING CODE: INT TO BOOL NODE: NO VALUE FOUND IN INPUT");
            ...
            writer.WriteLine($"bool {varName} = {(rawValue.Value != 0 ? "true" : "false")};");
        }
    }
}