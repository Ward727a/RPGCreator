...
public class StringToBoolNode : BaseNodeLogic
{
    public override FlowType FlowType => FlowType.Pure;
    public override string Title => "String to Bool";
    public override PipedPath Category { get; } = "Converter".ToPipedPath().Extend("String To");
    public override URN Urn { get; } = DefaultUrnModule.ToUrnModule("rpgc").ToUrn("string_to_bool_node");
    ...
    public override IReadOnlyList<IConnectorLogic> Outputs { get; set; } = new List<IConnectorLogic>
        { new BoolConnectorLogic("Converted value") };
    ...
    public override void GenerateCode(CodeContext context, IndentedTextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("// STRING TO BOOL");
        if (Inputs[0].RuntimeHasParent)
        {
            writer.WriteLine($"bool {varName} = bool.TryParse({input}.AsSpan(), out var result) && result;");
        }
        else
        {
            ...
            if (rawValue == null)
            {
                Logger.Error("ERROR WHILE GENERATING CODE: STRING TO BOOL NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine("// ERROR WHILE GENERATING CODE: STRING TO BOOL NODE: NO VALUE FOUND IN INPUT");
            ...
            writer.WriteLine($"bool {varName} = {bool.TryParse(rawValue.AsSpan(), out var result) && result};");
        }
    }
}