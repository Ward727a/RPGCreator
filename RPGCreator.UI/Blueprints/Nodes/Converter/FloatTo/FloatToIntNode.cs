...
public class FloatToIntNode : BaseNodeLogic
{
    public override FlowType FlowType => FlowType.Pure;
    public override string Title => "Float to Int";
    public override PipedPath Category { get; } = "Converter".ToPipedPath().Extend("Float To");
    public override URN Urn { get; } = DefaultUrnModule.ToUrnModule("rpgc").ToUrn("float_to_int_node");
    ...
    public override void GenerateCode(CodeContext context, IndentedTextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("// FLOAT TO INT");
        if(Inputs[0].RuntimeHasParent)
        {
            writer.WriteLine($"int {varName} = (int) {input};");
        }
        else
        {
            ...
            {
                Logger.Error("ERROR WHILE GENERATING CODE: FLOAT TO INT NODE: NO VALUE FOUND IN INPUT");
                writer.WriteLine("// ERROR WHILE GENERATING CODE: FLOAT TO INT NODE: NO VALUE FOUND IN INPUT");
            ...
            writer.WriteLine($"int {varName} = {(int)rawValue.Value};");
        }
    }
}