...
namespace RPGCreator.UI.Blueprints.Nodes.Math.Int;

public class AddNode : BaseNodeLogic
{
    public override FlowType FlowType => FlowType.Pure;
    public override string Title { get; } = "Add Integer";
    public override PipedPath Category { get; } = "Math".ToPipedPath().Extend("Integer");
    public override URN Urn { get; } = DefaultUrnModule.ToUrnModule("rpgc").ToUrn("math_int_add_node");

    public override IReadOnlyList<IConnectorLogic> Inputs { get; set; } =
        [new IntConnectorLogic("A"), new IntConnectorLogic("B")];
    public override IReadOnlyList<IConnectorLogic> Outputs { get; set; } = [new IntConnectorLogic("Result")];
    ...
    public override void GenerateCode(CodeContext context, IndentedTextWriter writer)
    {
        ...
        writer.WriteLine($"int {variableName} = {valueInput} + {valueInput2};");
    }
}